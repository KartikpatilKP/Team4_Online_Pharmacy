  ...
  const fetchMembers = async () => {
    try {
      ...
      Swal.fire({
        ...
        text: 'Please check your backend API.',
      });
    }
  };
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      const res = await axios.put(
        `http://localhost:8080/api/members/${id}/status`,
        null,
              ...
              {members.length === 0 ? (
                ...
                members.map((member) => (
                  <tr key={member.id}>
                    ...
                    <td>
                      <span
                        className={`status-pill status-${getStatus(member).toLowerCase()}`}
                      >
                        {getStatus(member)}
                      </span>
                    </td>
                    <td>
                      <div className="action-buttons">
                        ...
                        <button
                          ...
                          onClick={() => handleStatusChange(member.id, 'Declined')}
                        >
                          Decline
                        </button>
                        <button
